...
TMGA::TMGA(unsigned int _populationSize, unsigned int _demeSize, unsigned int _geneSize, float _recombinationRate, float _mutationRate, objectiveFunctionEvaluator *_eval, fitnessComparisonTypes comparisonType, int reportEvery) : 
...
void TMGA::evolve(unsigned int numIterations) {
    prepareToEvolve();
...
void TMGA::evolveUntil(float threshold) {
    prepareToEvolve();
    int i=0;
...
genotype& TMGA::getFittestIndividual() {
    return population[bestFitnessIndex];
}



unsigned char TMGA::bitwiseInfectAndMutate(const unsigned char a, const unsigned char b) {
    bitset<8> bitsA(a);
    bitset<8> bitsB(b);
    for(int i=0; i < 8; i++) {
...
void TMGA::genotypeToFloat(genotype &g, vector<float> &floatdata) {
